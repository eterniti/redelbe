...
typedef size_t (* VFType)(void *, void *, size_t, size_t);
typedef void *(* CLFType)(void *);
typedef void *(* RFType)(void *, void *, void *, uint32_t, void *, void *);
VFType vf_func;
VFType cf_func;
CLFType calls_l_func;
RFType request_files;
CLFType calls_dududu;

extern "C"
{

PUBLIC void SetupCD(CLFType orig)
{
	calls_dududu = orig;
}

PUBLIC void setup_vf(VFType orig)
{
	vf_func = orig;
}

PUBLIC void setup_cf(VFType orig)
{
	cf_func = orig;
}

PUBLIC void Setup_CLF(CLFType orig)
{
	calls_l_func = orig;
}

PUBLIC void Setup_RF(RFType orig)
{
	request_files = orig;
}

PUBLIC void *calls_dududu_patched(void *pthis)
{
	DPRINTF("CALLS_DUDUDU (called from %x, thread 0x%x)\n", Utils::DifPointer(BRA(0), GetModuleHandle(NULL)), (uint32_t)GetCurrentThreadId());
	return calls_dududu(pthis);
}

PUBLIC void *request_hair_patched(void *rcx, void *rdx, void *r8, uint32_t id, void *sp_20, void *sp_28)
{
	// MAR HAIR 001 -> 0x883e68da
	
	DPRINTF("REQUEST HAIR BEGIN 0x%x (thread 0x%x)\n", id, (uint32_t)GetCurrentThreadId());
	void *ret = request_files(rcx, rdx, r8, id, sp_20, sp_28);
	DPRINTF("REQUEST HAIR END\n");
	
	return ret;
}

PUBLIC void *calls_l_func_patched(void *pthis)
{
	DPRINTF("L_FUNC BEGIN (called from %x, thread 0x%x)\n", Utils::DifPointer(BRA(0), GetModuleHandle(NULL)), (uint32_t)GetCurrentThreadId());
	void *ret = calls_l_func(pthis);
	DPRINTF("L_FUNC END\n");
	
	return ret;
}

PUBLIC size_t vfinput_test(void *pthis, uint8_t *out, size_t buf_off,  size_t size)
{
	DPRINTF("VVV %p (mem %p) 0x%Ix (called from 0x%x) (buf_off %Ix)\n", pthis, out+buf_off, size, Utils::DifPointer(BRA(0), GetModuleHandle(NULL)), buf_off);
	size_t ret = vf_func(pthis, out, buf_off, size);
	
	return ret;
}
static uint8_t mh_001[300] = {
	
};

PUBLIC size_t cfinput_test(uint8_t *pthis, uint8_t *out, size_t buf_off,  size_t size)
{
	uint8_t *in_buf = *(uint8_t **)&pthis[0x40];
	
	DPRINTF("CCC %p (mem %p) 0x%Ix (called from 0x%x) (buf_off %Ix) (inbuf = %p)\n", pthis, out+buf_off, size, Utils::DifPointer(BRA(0), GetModuleHandle(NULL)), buf_off, in_buf + *(uint32_t *)&pthis[0x7C]);
	size_t ret = cf_func(pthis, out, buf_off, size);
	
	if (size >= sizeof(mh_001) && memcmp(out+buf_off, mh_001, sizeof(mh_001)) == 0)
	{
		DPRINTF("BINGUI\n");
	}
	else if (size >= sizeof(mh_001) && memcmp(in_buf + *(uint32_t *)&pthis[0x7C], mh_001, sizeof(mh_001)) == 0)
	{
		DPRINTF("BINGUW\n");
	}
