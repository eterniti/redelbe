...
#define DEBUG_FILE	"REDELBE/redelbe_log.txt"
...
int __attribute__ ((format (printf, 1, 2))) DebugPrintf(const char* fmt, ...) 
{
	...
	{
		MessageBoxA(NULL, dbg, "REDELBE", 0);
	}
...
int __attribute__ ((format (printf, 2, 3))) FatalPrintf(bool ask, const char* fmt, ...) 
{
	char *dbg;
	va_list ap;
	...
	if (ask)
	{
		if (MessageBoxA(NULL, "Do you want to abort game load?", "REDELBE", MB_YESNO | MB_ICONEXCLAMATION) != IDYES)
			lve = false;
	}
